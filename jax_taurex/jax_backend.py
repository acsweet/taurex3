...
def prepare_contributions(taurex_model, wngrid):
    ...
    for contrib in taurex_model.contribution_list:
        if isinstance(contrib, AbsorptionContribution):
            ...
            # Skip SimpleCloudsContribution as we handle it dynamically in path_integral
            if contrib.__class__.__name__ == 'SimpleCloudsContribution':
                print(f"Skipping {contrib.__class__.__name__} in prepare_contributions (handled dynamically)")
                continue
            
            # Skip LeeMieContribution as we handle it dynamically in path_integral
            if contrib.__class__.__name__ == 'LeeMieContribution':
                print(f"Skipping {contrib.__class__.__name__} in prepare_contributions (handled dynamically)")
                continue
...
def path_integral(wngrid, params, absorption_sigmas_tuple, dynamic_data, static_data):
    """
    Path integral with proper CIA handling (ρ² dependence) and optional clouds.
    
    If params contains 'clouds_pressure', applies gray cloud opacity at that pressure level.
    Clouds set tau to a large value (100) where P >= P_clouds, making deeper layers opaque.
    """
    ...
    # Lee Mie Contribution (Dynamic)
    if 'lee_mie_radius' in params:
        # Extract parameters
        a = params['lee_mie_radius']
        q_mie = params['lee_mie_q']
        lee_mie_mix = params['lee_mie_mix_ratio']
        bottom_p = params.get('lee_mie_bottomP', 1e6)
        top_p = params.get('lee_mie_topP', 1e-2)
        
        # Wavelength in microns
        wl_um = 10000.0 / wngrid
        
        # x = 2 pi a / lambda
        x = 2.0 * jnp.pi * a / wl_um
        
        # Q_ext
        q_ext = 5.0 / (q_mie * x**(-4.0) + x**(0.2))
        
        # Sigma (m2)
        # a is in um, so convert to m for area
        a_m = a * 1e-6
        sigma_mie = q_ext * jnp.pi * (a_m**2.0)
        
        # Pressure mask (differentiable)
        # sigmoid(P - top) * sigmoid(bottom - P)
        # Use log pressure for better numerics
        log_P = jnp.log(jnp.maximum(pressure_profile, 1e-20))
        log_top = jnp.log(jnp.maximum(top_p, 1e-20))
        log_bot = jnp.log(jnp.maximum(bottom_p, 1e-20))
        
        k = 10.0 # Steepness
        mask = jax.nn.sigmoid(k * (log_P - log_top)) * jax.nn.sigmoid(k * (log_bot - log_P))
        
        # Calculate effective cross-section: sigma_eff = (sigma_mie * N_cloud) / N_atm
        # N_cloud = lee_mie_mix (particles/m3)
        # N_atm = density_profile (particles/m3)
        
        lee_mie_extinction = mask[:, None] * sigma_mie[None, :] * lee_mie_mix
        lee_mie_sigma_eff = lee_mie_extinction / (density_profile[:, None] + 1e-30)
        
        regular_sigma = regular_sigma + lee_mie_sigma_eff
    ...
    # 9. Apply clouds if clouds_pressure parameter is present
    # Gray clouds: set tau to large value where P >= P_clouds
    # This blocks all light from deeper layers
    if 'clouds_pressure' in params:
        clouds_pressure = params['clouds_pressure']
        
        # Differentiable cloud implementation using sigmoid
        # We want opacity ~ 100 when P > P_clouds (deeper)
        # and opacity ~ 0 when P < P_clouds (higher up)
        # log(P) increases with depth.
        # x = log(P) - log(P_clouds)
        # When x > 0 (deeper), we want sigmoid(x) -> 1
        # When x < 0 (higher), we want sigmoid(x) -> 0
        ...
        # Steepness factor k. 
        # k=10 means transition happens over ~1 decade of pressure
        # Reduced from 50 to 10 to improve stability
        k = 10.0 
        
        cloud_opacity_factor = jax.nn.sigmoid(k * (log_P - log_P_cloud))
        
        # Apply large optical depth where clouds exist
        # Broadcast over wavelength dimension
        cloud_tau = cloud_opacity_factor[:, None] * 100.0
